...
export class I18nExtractorPlugin extends BaseExtractor<I18nAnalysisResult, I18nExtractionSummary> {
  ...
  private async findUntranslatedStrings(ast: any, content: string, config: I18nExtractorConfig): Promise<I18nAnalysisResult['untranslatedStrings']> {
    ...
    ASTUtils.traverse(ast, {
      ...
      JSXText: (path: any) => {
        ...
        if (this.isTranslatableString(text, config)) {
      ...
      StringLiteral: (path: any) => {
        if (!config.analyzeStringLiterals) return;

        const text = path.node.value;
        if (this.isTranslatableString(text, config) && !this.isInTranslationCall(path)) {
          untranslatedStrings.push({
            text,
            type: 'string-literal',
            line: path.node.loc?.start.line || 0,
  ...
  private isTranslatableString(text: string, config: I18nExtractorConfig): boolean {
    // Length check
    if (text.length < config.minStringLength!) return false;
